...
class DangerChecks {
    constructor(opts) {
        ...
        this.checkPRDescription = () => {
            ...
            if (body.length < 10) {
                warn('<i>Your PR description seems a bit short. Here are some things to consider: describe your changes, include helpful steps for reviewers, link to Clubhouse ticket, and add a commit code block at the end (if auto-merging).</i>');
            }
            ...
            if (codeBlocks) {
                ...
                fail('It looks like you forgot to include a commit code block at the end of your PR description. Simply create a code block that starts with <i>```commit</i> containing your commit body. I use this as your commit body once your PR is ready to be merged.');
            }
        };
        ...
        this.autoMergePullRequest = (manualMergeTag) => {
            ...
            if (!locked && !merged && mergeable && mergeable_state !== 'blocked' && rebaseable) {
                ...
                if (!this.mergeCommitBlock) {
                    fail("Oops! It looks like you're missing a commit block in the description of your PR. Simply create a code block at the bottom of your description with your commit message. I use this when I auto-merge your PR.");
                    return;
                }
        ...
        this.addMetaDataAboutPR = () => {
            ...
            message(`This PR contains ${commits} commits affecting ${changed_files} files, for a total of ${additions} additions and ${deletions} deletions.`);
        };
        ...
        this.performSpellCheck = (str, location) => {
            ...
            return str.split(' ').forEach(word => {
                ...
                if (misspelled) {
                    warn(`Potential typo in ${location}: <b>${word}</b>.<br/> Did you maybe mean one of these: <i>${suggestions.join(', ')}</i>?`);
                }
            });
        };
